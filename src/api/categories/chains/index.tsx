...
export async function getChainPageData(chain?: string) {
	const chainMetadata = chainsMetadata['sonic'] ?? null
	...
	const [
		...
		getPeggedOverviewPageData(chainMetadata?.name)
			.then((data) => {
	...
	const protocolsList = formatProtocolsList({
		protocols: filteredProtocols.filter(p => p.chain === 'Sonic'),
	...
	return {
		props: {
			...
			chainsSet: chains,
			chainOptions: [{label: 'Sonic', to: '/chain/sonic'}], // Only Sonic chain is available
			protocolsList,
			volumeData: {
