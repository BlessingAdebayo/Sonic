...
import { SonicOnlyFilter } from '~/components/SonicOnlyFilter'
...
export function BridgesOverview({
	selectedChain = 'Sonic',
	chains = [],
	filteredBridges,
	bridgeNames,
	...
	const chainOptions = [{ label: 'Sonic', to: handleRouting('Sonic') }] // Only show Sonic data
	...
	return (
		<>
			<SonicOnlyFilter data={filteredBridges}>
				{(filteredData) => (
			...
			{isBridgesShowingTxs && (
				<LargeTxsTable 
					data={largeTxsData.filter(d => d.chain === 'Sonic' || d.chains?.includes('Sonic'))} 
					chain="Sonic" 
				/>
			)}
			{!isBridgesShowingTxs && (
  <BridgesTable data={filteredBridges.filter(d => d.chain === 'Sonic' || d.chains?.includes('Sonic'))} />
)}
		</>
	)
}
