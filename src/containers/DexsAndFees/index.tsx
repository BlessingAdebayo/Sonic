...
import { filterSonicOnly } from '~/utils/sonicFilter'
		...
		const protocolsList = groupProtocolsByParent({
			protocols:
				categoriesToFilter.length > 0
					? filterSonicOnly(props.protocols).filter((p) => (p.category ? selectedCategories.includes(p.category) : false))
					: filterSonicOnly(props.protocols),
