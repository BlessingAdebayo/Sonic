	...
	React.useEffect(() => {
		...
		const option = {
			...
			tooltip: {
				...
				formatter: function (params) {
					...
					if (params.seriesName !== 'Volume') {
						...
						vals +=
							'<li style="list-style:none">' +
							'<span style="display:inline-block;margin-right:4px;border-radius:10px;width:10px;height:10px;background-color:#00fff0;"></span>' +
							'Moving Average:' +
							'&nbsp;&nbsp;' +
			...
			xAxis: {
				...
				axisLine: {
					lineStyle: {
						color: isDark ? 'rgb(165, 236, 192)' : 'rgba(0, 0, 0, 1)',
						opacity: 0.2
					}
				},
				splitLine: {
			...
			yAxis: [
				{
					...
					axisLine: {
						lineStyle: {
							color: isDark ? 'rgb(172, 240, 187)' : 'rgba(0, 0, 0, 1)',
							opacity: 0.1
						}
					},
					boundaryGap: false,
					nameTextStyle: {
						...
						fontSize: 14,
						fontWeight: 400,
						color: isDark ? 'rgb(177, 233, 199)' : 'rgba(0, 0, 0, 1)'
					},
					splitLine: {
						lineStyle: {
							color: '#94e3ba',
