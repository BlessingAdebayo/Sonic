...
import { DEFAULT_ASSETS_LIST, DEFAULT_CHART_DATA, FALLBACK_AVAILABILITY } from '~/constants/build-defaults'
import { liquidationsIconUrl } from '..'
import { fetchDefillamaApi } from '~/utils/api/defillama'
...
async function fetchWithFallback(url: string) {
  try {
    const response = await fetchWithRetry(url, {}, DEFAULT_CONFIG)
...
export const LOCAL_DEFAULT_ASSETS_LIST: ISearchItem[] = DEFAULT_ASSETS_LIST_RAW.map(({ name, symbol }) => ({
	name,
	symbol,
